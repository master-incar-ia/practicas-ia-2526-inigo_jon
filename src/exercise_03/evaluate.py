...
def evaluate_and_plot(loader, model, dataset_name, output_folder):
    ...
    # Try to denormalize inputs for plotting if dataset provides helper
    base_dataset = getattr(loader.dataset, "dataset", loader.dataset)
    if hasattr(base_dataset, "denormalize_x"):
        try:
            all_inputs = base_dataset.denormalize_x(all_inputs)
        except Exception:
            pass
...
if __name__ == "__main__":
    ...
    # Create an instance of the dataset (raw values)
    dataset = NoisyRegressionDataset(size=10000)

    # If normalization parameters were saved during training, load and apply them
