...
def train_model(output_folder: Path, device: torch.device):
    ...
    dataset_train = CIFAR10Dataset("./data", train=True, transform=transform_train)
    dataset_val = CIFAR10Dataset("./data", train=False, transform=transform_val)
    ...
    model = MLPClassifier(input_size=3072, output_dim=10, hidden_sizes=[512, 256, 128]).to(device)
    ...
    for epoch in tqdm(range(num_epochs)):
        ...
        for inputs, targets in train_loader:
            ...
            train_total += targets_cuda.size(0)
            train_correct += (predicted == targets_cuda).sum().item()
        ...
        with torch.no_grad():
            for inputs, targets in val_loader:
                ...
                val_loss += loss.item()
                _, predicted = torch.max(outputs.data, 1)
                val_total += targets_cuda.size(0)
                val_correct += (predicted == targets_cuda).sum().item()
